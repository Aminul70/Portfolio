...
export function FooterEditor() {
  ...
  return <div className="h-full flex flex-col pt-2 font-['Segoe_UI',sans-serif]">
    <div className="px-4 pb-2 border-b border-white/10 flex items-center justify-between">
      <h2 className="text-lg font-semibold text-white">Footer Configuration</h2>
    </div>

    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      <div className="space-y-1">
        <label className="text-xs text-gray-400">Terminal Quote</label>
        <input
          type="text"
          value={content.footer.terminalQuote}
          onChange={e => setContent({ ...content, footer: { ...content.footer, terminalQuote: e.target.value } })}
          className="w-full bg-black/40 border border-white/10 rounded-sm px-2 py-1.5 text-white text-sm focus:border-cyan-500 outline-none font-mono"
        />
      </div>

      <div className="space-y-1">
        <label className="text-xs text-gray-400">Copyright Text</label>
        <input
          type="text"
