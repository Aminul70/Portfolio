...
import { SettingsEditor, WALLPAPERS } from '../../components/admin/editors/SettingsEditor';
import './admin-cursor.css';

type WindowType = 'projects' | 'hero' | 'about' | 'contact' | 'footer' | 'settings';
...
export function Dashboard() {
  ...
  const getWallpaperStyle = () => {
    if (currentWallpaper.startsWith('custom:')) {
      const url = currentWallpaper.replace('custom:', '');
      return {
    ...
    return wallpaper?.style || WALLPAPERS[0].style;
  };

  return <div className="admin-cursor h-screen w-full overflow-hidden relative font-['Segoe_UI',sans-serif] select-none" style={{ cursor: 'url(/custom-cursor.png) 12 12, auto' }}>
    {/* Desktop Wallpaper */}
    <div className="absolute inset-0 bg-cover bg-center transition-all duration-500" style={getWallpaperStyle()}>
      {/* Light Rays for default wallpaper */}
      {currentWallpaper === 'default' && <div className="absolute inset-0 overflow-hidden opacity-40">
      ...
      {/* Windows Logo for default wallpaper */}
      {currentWallpaper === 'default' && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-64 h-64 grid grid-cols-2 gap-4 opacity-30">
    ...
    <AnimatePresence>
      {Object.values(windows).map(window => window.isOpen && !window.isMinimized && <AeroWindow key={window.id} window={window} isActive={activeWindowId === window.id} onClose={() => closeWindow(window.id)} onMinimize={() => minimizeWindow(window.id)} onMaximize={() => maximizeWindow(window.id)} onFocus={() => focusWindow(window.id)}>
        ...
        {window.id === 'settings' && <SettingsEditor currentWallpaper={currentWallpaper} onWallpaperChange={setCurrentWallpaper} />}
      </AeroWindow>)}
    </AnimatePresence>
    ...
    <div className="fixed bottom-0 left-0 right-0 h-12 bg-black/20 backdrop-blur-xl border-t border-white/10 flex items-center px-1 z-50 shadow-[0_-2px_10px_rgba(0,0,0,0.3),inset_0_1px_0_rgba(255,255,255,0.1)]">
      ...
      <button
        ...
        className="relative w-16 h-full flex items-center justify-center group px-1"
      >
        <img
          src="/remove-photos-background-removed(2).png"
          alt="Start"
          className="h-[115%] w-auto object-contain transition-all duration-200 group-hover:drop-shadow-[0_0_8px_rgba(59,130,246,0.6)] group-active:scale-95 group-active:brightness-90"
        />
      </button>
      ...
      <div className="flex-1 flex items-center gap-1 px-2">
        {Object.values(windows).filter(w => w.isOpen).map(window => <button key={window.id} onClick={() => window.isMinimized ? focusWindow(window.id) : activeWindowId === window.id ? minimizeWindow(window.id) : focusWindow(window.id)} className={`h-8 px-3 rounded flex items-center gap-2 border transition-all min-w-[140px] max-w-[200px] ${activeWindowId === window.id && !window.isMinimized ? 'bg-white/30 border-white/40 shadow-inner' : 'bg-white/10 border-white/20 hover:bg-white/20'}`}>
          <div className="w-4 h-4">
            ...
            {window.id === 'settings' && <Settings size={16} className="text-cyan-300" />}
          </div>
    ...
    <AnimatePresence>
      {showStartMenu && <>
        ...
        <motion.div initial={{
          ...
          y: 10,
          scale: 0.95
        }} className="absolute bottom-11 left-0 w-[420px] h-[520px] bg-gradient-to-b from-slate-900/95 to-slate-800/95 backdrop-blur-xl border border-white/40 rounded-tr-lg shadow-2xl z-50 flex overflow-hidden">
          ...
          {/* Right Column - Places */}
          <div className="w-[160px] bg-slate-900/50 p-2 text-white flex flex-col text-sm space-y-1">
            ...
            <StartPlaceItem icon={<Settings size={16} />} label="Settings" onClick={() => openWindow('settings')} />
...
function StartPlaceItem({
  ...
  return <div onClick={onClick} className="flex items-center gap-2 px-2 py-1.5 hover:bg-white/10 rounded cursor-pointer text-white/90 hover:text-white transition-colors text-xs">
    {icon}
    <span>{label}</span>
  </div>;
}
function AeroWindow({
  ...
  return <motion.div drag={!window.isMaximized} dragControls={dragControls} dragListener={false} dragMomentum={false} initial={{
    ...
    backgroundColor: isActive ? 'rgba(30, 41, 59, 0.95)' : 'rgba(30, 41, 59, 0.9)',
    ...
    <div onPointerDown={e => dragControls.start(e)} className={`h-8 px-3 flex items-center justify-between select-none cursor-move ${isActive ? 'bg-gradient-to-b from-white/30 via-white/20 to-white/10' : 'bg-gradient-to-b from-white/15 via-white/10 to-white/5'}`} onDoubleClick={onMaximize} style={{
      ...
      <div className="flex items-center gap-2">
        <div className="w-4 h-4">
          ...
          {window.id === 'settings' && <Settings size={14} className="text-cyan-400 drop-shadow" />}
        </div>
        <span className={`text-xs font-normal ${isActive ? 'text-white' : 'text-gray-400'}`}>
          {window.title}
        </span>
      </div>
      ...
      <div className="flex items-center gap-0.5">
        <button onClick={e => {
          e.stopPropagation();
          onMinimize();
        }} className="w-7 h-6 flex items-center justify-center hover:bg-white/10 rounded-sm transition-colors">
