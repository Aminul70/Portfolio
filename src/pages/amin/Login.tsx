...
import { signInWithGoogle, isAuthorizedEmail, getRandomDenialMessage } from '../../lib/firebase';
...
export function Login() {
  ...
  const handleProfileClick = async () => {
    ...
    try {
      ...
      if (isAuthorizedEmail(user.email)) {
        ...
        // Unauthorized - show funny denial message
        const denialMessage = getRandomDenialMessage();
        setError(denialMessage);

        // Sign out the unauthorized user
        // We'll import signOutUser if needed, but for now just show error
  ...
  return <div className="admin-cursor min-h-screen flex flex-col items-center justify-center relative overflow-hidden" style={{
    ...
    <motion.div initial={{
      ...
      <motion.button onClick={handleProfileClick} disabled={isLoading} whileHover={{
        scale: isLoading ? 1 : 1.05,
        filter: isLoading ? 'brightness(0.8)' : 'brightness(1.1)'
      }} whileTap={{
        scale: isLoading ? 1 : 0.95
      }} className="relative cursor-pointer focus:outline-none focus:ring-2 focus:ring-cyan-500/50 rounded-2xl disabled:cursor-not-allowed">
        <div className="w-[140px] h-[140px] rounded-2xl border-2 border-white/5 shadow-2xl shadow-cyan-500/10 overflow-hidden">
          <img src="/27ce93bd-e406-4e40-80bc-300b0680ca3f.png" alt="Admin Profile" className={`w-full h-full object-cover ${isLoading ? 'opacity-50' : ''}`} />
          {isLoading && (
      ...
      <AnimatePresence>
        {error && <motion.div initial={{
          ...
          opacity: 0,
          y: -10
        }} className="text-red-500 font-medium mt-3 text-center max-w-sm">
          {error}
        </motion.div>}
      </AnimatePresence>
    </motion.div>
